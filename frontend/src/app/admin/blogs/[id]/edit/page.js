...
export default function EditBlogPage() {
  ...
  const [uploadedImageUrl, setUploadedImageUrl] = useState(""); // ← Add this
  ...
  const fetchBlog = async () => {
    try {
      const response = await axiosInstance.get(`/blogs/id/${blogId}`);
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      setImage(imageUrl); // Store URL instead of base64
      toast.success("Image uploaded successfully", { id: loadingToast });
    } catch (error) {
      toast.error(error.message || "Failed to upload image", {
        id: loadingToast,
      });
      setImagePreview(currentImage); // Revert to original image
      setUploadedImageUrl("");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (uploadedImageUrl) {
        ...
        // Image was removed - send empty or handle error
                  ...
                  {imagePreview && (
                    <div className="mt-3">
                      ...
                      <button
                        ...
                        className="cursor-pointer mt-2 px-3 py-1 text-sm font-medium text-red-600 border border-red-600 rounded-md hover:bg-red-50 disabled:opacity-50"
                      >
                        Remove Image
                      </button>
                    </div>
            ...
            <div className="bg-white shadow rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  disabled={updating}
                  className="px-3 py-1 text-sm font-medium text-indigo-600 border border-indigo-600 rounded-md hover:bg-indigo-100 disabled:opacity-50"
                >
                  + Add FAQ
                </button>
              </div>
